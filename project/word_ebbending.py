...
def clean_to_vectors(doc):
    special_word = ['pra', "para", "e", "E", "", " ", "O", "A", "a", "o", 'bom', 'excelente', 'bem', 'agradavel',
                    'otimo',"Ótimo",'boa',"ótimo",'Bom','Boa','amazing','Amazing','\n']

    text_1 = [token for token in doc if
              token.text not in special_word and token.is_punct == False and token.is_stop == False and token.is_bracket == False]
...
def get_word_vector(dados):
    ...
    dados['Docs_vector'] = dados['Docs_clean'].apply(lambda x: clean_to_vectors(
...
#     dados['Comment_vector'] = dados['Docs_clean'].apply(lambda x: clean_to_vectors(
#         x))  # cada linha sao vetores das palavras lematizadas, sem pontuacao e stopwords, de cada comentario
...
def get_comment_vector_cos(dados):
    ...
    for i_comment_matrix in range(len(comment_matrix)):
        cos = np.vdot(comment_matrix[i_comment_matrix], ref) / (
                    (np.linalg.norm(comment_matrix[i_comment_matrix])) * np.linalg.norm(ref))
...
def get_comment_vector_cos_v2(dados):
    ...
    for i_comment_matrix in range(len(comment_matrix)):
        ...
        for i_dimension in range(len(comment_matrix[i_comment_matrix]) - 1):
            pair_vector = [comment_matrix[i_comment_matrix][i_dimension],
                           comment_matrix[i_comment_matrix][i_dimension + 1]]
    ...
    for i_comment_matrix in range(len(comment_matrix)):
        ...
        for i_dimension in range(len(comment_matrix[i_comment_matrix]) - 2):
            ...
            while (second_dimension <= len(comment_matrix[i_comment_matrix]) - 1):
                pair_vector = [comment_matrix[i_comment_matrix][i_dimension],
                               comment_matrix[i_comment_matrix][second_dimension]]
                cos = np.vdot(pair_vector, ref) / ((np.linalg.norm(pair_vector)) * np.linalg.norm(ref))
                comment_matrix_cos[i_comment_matrix].append(cos)
...
def similarity_matrix(dados):
    ...
    for i_comment_matrix in range(len(comment_matrix)):
        ...
        for i_comment_matrix_2 in range(len(comment_matrix)):
            comment = []
            cos = np.vdot(comment_matrix[i_comment_matrix], comment_matrix[i_comment_matrix_2]) / (
                        (np.linalg.norm(comment_matrix[i_comment_matrix])) * np.linalg.norm(
                    comment_matrix[i_comment_matrix_2]))
